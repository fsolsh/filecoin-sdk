package com.fsolsh.mining.filecoin.handler;
...
import com.fsolsh.mining.filecoin.crypto.ECKey;
import com.fsolsh.mining.filecoin.exception.*;
import com.fsolsh.mining.filecoin.model.result.*;
import com.fsolsh.mining.filecoin.utils.Convert;
...
import com.fsolsh.mining.filecoin.model.RpcPar;
...
public class FilecoinHandler {
    ...
    private String execute(RpcPar par, int timeout) throws ExecuteException {
        HttpRequest post = HttpUtil.createPost(NodeConfig.getINSTANCE().getUrl());
        if (NodeConfig.getINSTANCE().getNodeType().equals(FilecoinCnt.PRIVATE_NODE)) {
            post.header("Authorization", NodeConfig.getINSTANCE().getNodeAuthorization());
        } else if (NodeConfig.getINSTANCE().getNodeType().equals(FilecoinCnt.INFURA_NODE)) {
            post.basicAuth(NodeConfig.getINSTANCE().getRpcUserName(), NodeConfig.getINSTANCE().getRpcSecret());
        }
    ...
    public SendResult send(Transaction transaction, String privateKey, int timeout) throws SendException, ExecuteException {
        if (transaction == null || StrUtil.isBlank(transaction.getFrom())
            ...
            throw new SendException("parameter cannot be empty");
        }
        ...
        params.add(json);
        RpcPar rpcPar = RpcPar.builder().id(1).jsonrpc("2.0").method(FilecoinCnt.BOARD_TRANSACTION).params(params).build();
